        ...
        {week.map(({ month, day }, dayIndex) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute bottom-1 left-1 right-1 top-10 overflow-y-auto">
                {dayEvents.map(event => (
                  <div key={event.id} className="text-xs text-blue-500 sm:text-sm">
                    {event.title}
                  </div>
                ))}
              </div>
              <button type="button" className="absolute right-2 top-2 rounded-full opacity-0 transition-all focus:opacity-100 group-hover:opacity-100" onClick={(e) => { e.stopPropagation(); setShowAddEventModal(true); setNewEventDate(new Date(year, month, day)); }}>
          ...
          <div className="rounded-lg bg-white p-6 shadow-xl">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="mr-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100"
              >
                Cancel
              </button>
              <button
                onClick={handleAddEvent}
                className="rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-gradient-to-bl focus:outline-none focus:ring-4 focus:ring-cyan-300"
              >
                Add Event
              </button>
            </div>
